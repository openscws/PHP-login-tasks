<?php
   include('session.php');
?>
<html">
   
   <head>
      <title>Welcome </title>
   </head>
   
   <body>
   
   <?php 
   
   if($_SERVER["REQUEST_METHOD"] == "POST") {
	 
      $taskid = mysqli_real_escape_string($db,$_POST['task']);
	 
	  $sql = "UPDATE task SET isCompleted=1 WHERE id=$taskid;";
       mysqli_query($db,$sql);
	 
 }
   
   
   echo "<h1>Welcome  $login_session </h1>";
   $user=$_SESSION['login_user'];
   $sql = "SELECT * FROM admin WHERE username='$user'";
   $result = mysqli_query($db,$sql);
   $row = mysqli_fetch_array($result,MYSQLI_ASSOC);
   $myuser=$row;
		if($row['isAdmin']==1){
			echo "
			<p><a href = \"create.php\">Create new task</a></p>
	  <p><a href = \"edit.php\">Edit existing task</a></p>
	  <p><a href = \"delete.php\">Delete existing task</a></p>
      
			
			";
		}
		else {
			
$sql = "SELECT * FROM task";
$result = mysqli_query($db,$sql);
$tasks = array();
while($row = mysqli_fetch_assoc($result)){
  $tasks[] = $row;
}
//get users
$sql = "SELECT * FROM admin";
$result = mysqli_query($db,$sql);
$users = array();
while($row = mysqli_fetch_assoc($result)){
  $users[] = $row;
 
}
echo "
			<form action = \"\" method = \"post\">
                 
                  
				  <select id=\"task\" name=\"task\">";
foreach($tasks as $k=>$val){
						if($tasks[$k]['isCompleted']==0 and $tasks[$k]['adminid']==$myuser['id']){
						echo "<option value=\"".$tasks[$k]['id']."\">".$tasks[$k]['task'];
						
						foreach($users as $g=>$val2){
						if($tasks[$k]['adminid']==$users[$g]['id']){
							echo " [".$users[$g]['username']."]";
						}
						}
						echo "</option>";
						}
					}
echo	"  </select>
					
                  <input type = \"submit\" value = \" Mark completed \"/><br />
               </form>
      
			";

		}
   ?>
       
	  <h2><a href = "logout.php">Sign Out</a></h2>
	  
   </body>
   
</html>